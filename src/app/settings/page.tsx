...
export default function SettingsPage() {
  ...
  const addToast = useCallback((message: string, type: ToastMessage["type"]) => {
    setToasts((prev) => [...prev, { id: createToastId(), message, type }]);
  }, []);
  ...
  useEffect(() => {
    ...
    ]).then(([usageData, bvData]) => {
      setUsage(usageData);
      if (bvData.brandVoice) setBrandVoice(bvData.brandVoice);
    });
  }, []);
  ...
  async function handleBillingPortal() {
    ...
    if (data.url) {
      ...
      addToast("No billing account found. Please subscribe first.", "error");
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-sm px-4 py-2 rounded-lg font-medium transition-all duration-200 border bg-amber-500/20 border-amber-500/30 text-amber-300 hover:bg-amber-500/30 disabled:opacity-50"
                >
                  {brandVoiceSaving ? "Saving..." : "Save Brand Voice"}
                </button>
              </div>
            </div>
          ) : (
