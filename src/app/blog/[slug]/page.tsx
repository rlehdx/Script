      ...
      <main className="min-h-screen bg-bg-primary pt-24 pb-20">
        <div className="max-w-2xl mx-auto px-5">
          ...
          <div className="mt-10 mb-8 flex items-center gap-3">
            <span className="text-xs text-slate-600 uppercase tracking-wider">Share</span>
            <a
              href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(`${APP_URL}/blog/${slug}`)}&via=scriva_app`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-white border border-white/8 hover:border-white/20 rounded px-3 py-1.5 transition-colors"
            >
            ...
            <a
              href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`${APP_URL}/blog/${slug}`)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-white border border-white/8 hover:border-white/20 rounded px-3 py-1.5 transition-colors"
            >
          ...
          {/* Related guides — same tag first, then others */}
          <div className="mt-4 mb-8">
            ...
            <div className="flex flex-col gap-2">
              {[
                ...Object.entries(POSTS).filter(([s, p]) => s !== slug && p.tag === post.tag),
                ...Object.entries(POSTS).filter(([s, p]) => s !== slug && p.tag !== post.tag),
              ]
                .slice(0, 3)
                .map(([s, p]) => (
